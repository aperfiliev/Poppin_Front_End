(function($) {
	$.fn.gpSlide = function(opts){
		...
		var defaults = {
			...
			prev: true,
			next: true,
			responsive: false, //Required CSS new Width
			autorun: true,
			animation: 'slide'
		};
		...
		return this.each(function(){
			...
			var interfaceNew = new interface.create(self);
			...
			slide = {
				...
				next: function(obj) {
					...
					if(opts.goto) {
						...
						$(obj.btn[next_btn]).addClass('active');
					...
					if(opts.animation == 'fade'){
						...
						$(active_item).animate({opacity: 0}, opts.animationSpeed, function(){
							...
							obj.items.removeClass('active').css({'z-index': 'auto'});;
							$(next_item).addClass('active').css({'z-index': 2});
				...
				prev: function(obj) {
					...
					if(opts.goto) {
						...
						$(obj.btn).removeClass('active');
						$(obj.btn[prev_btn]).addClass('active');
					...
					if(opts.animation == 'fade'){
						...
						$(active_item).animate({opacity: 0}, opts.animationSpeed, function(){
							...
							obj.items.removeClass('active').css({'z-index': 'auto'});;
							$(prev_item).addClass('active').css({'z-index': 2});
				...
				goto: function(pos, obj) {
					...
					if(opts.animation == 'fade'){
						...
						$(active_item).animate({opacity: 0}, opts.animationSpeed, function(){
							...
							obj.items.removeClass('active').css({'z-index': 'auto'});;
							$(goto_item).addClass('active').css({'z-index': 2});
...
$j(function(){
	...
	if($j('.slide-wrap ul li').length > 0){
		//$j('.slide-wrap ul').css({visibility: 'hidden' });
