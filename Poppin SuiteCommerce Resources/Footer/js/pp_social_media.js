...
jQuery(document).ready(function() {
	...
	if (window.location.href.indexOf(poppinres.url.poppincheckout) < 0) {
...
jQuery('div#waitmask').hide();
				...
				case 'error':
					...
					if(errormessage.indexOf('password')!=-1){
						powerTip.create('minipassword', errormessage, 'powerTipminipassword', -52, 100);
						jQuery('#minipassword').on('focusin', function() { powerTip.hide('powerTipminipassword'); });
						jQuery('#minipassword').attr('class', 'input-red');
					} else if(errormessage.indexOf("haven't registered yet")!=-1) {
						powerTip.create('miniemail', errormessage, 'powerTipminiemail', -52, 100);
						jQuery('#miniemail').on('focusin', function() { powerTip.hide('powerTipminiemail'); });
						jQuery('#miniemail').attr('class', 'input-red');
					}
					else if(errormessage.indexOf("already exists")!=-1) {
						powerTip.create('miniemailregnew', errormessage, 'powerTipminiemailreg', -52, 100);
						jQuery('#miniemailregnew').on('focusin', function() { powerTip.hide('powerTipminiemailreg'); });
						jQuery('#miniemailregnew').attr('class', 'input-red');
					}
					else {
...
function miniForgotPassword() {
	...
	jQuery('#miniemail').removeClass('input-red');
	if (user.email === '') {
...
function forgotPasswordOld() {
	...
	jQuery('#forgot-pass-form').on('submit', function(e) {
		e.preventDefault();
		if (jQuery("form#forgot-pass-form > fieldset > input#submit.orangeBtn").val() == 'Continue') {
...
function resetPasswordOld(){
	var formHtml = '<div class="new-customer" style="position:relative;"><form id="reset-pass-form" action="sendResetPasswordRequest()">'
		+ '<fieldset style="border: 0px none;">' 
	...
	jQuery('#reset-pass-form').on('submit', function(e) {
		e.preventDefault();
		if (jQuery("form#reset-pass-form > fieldset > input#submit.orangeBtn").val() == 'Continue') {
...
function sendResetPasswordRequestOld() {
	...
	var pass = document.forms['reset-pass-form'].elements['resetpass'].value;
	var regPass		= /^[0-9a-zA-Z]{6,}$/;
	if(!regPass.test(pass))
	{
		powerTip.create('resetpass', poppinres.text.passwordvalidation, 'powerTipResetPass', -44, 0);
		jQuery('#resetpass').on('focusin', function() { powerTip.hide('powerTipResetPass'); }) ;
		jQuery('#resetpass').attr( "class", "input-red");
		return false;
	}
...
jQuery('div#waitmask').hide();
	...
	jQuery("form#reset-pass-form > fieldset > input#submit.orangeBtn").val("Close");
	jQuery("form#reset-pass-form > fieldset > p.message").html(message);
}
...
jQuery('div#waitmask').hide();
	...
	jQuery("form#forgot-pass-form > fieldset > input#submit.orangeBtn").val("Close");
	jQuery("form#forgot-pass-form > fieldset > p.message").html(message);
}
/*
...
jQuery(document).ready(function(){
	jQuery('#minicompanyhelp').on('mouseenter', function(){
		powerTip.create('minicompany', jQuery('#miniwhatthisfortext').html(), 'powerTipCompany', -17, 176);
	});
...
function miniloginSubmit(loginUser)
{
	...
	jQuery('#minipassword, #miniemail').removeClass('input-red');
	powerTip.hide('powerTipminiemail');
	powerTip.hide('powerTipminipassword');
	
	if (loginUser.email === '') {
		powerTip.create('miniemail', poppinres.text.emailempty, 'powerTipminiemail', -37, 100);
		jQuery('#miniemail').on('focusin', function() { powerTip.hide('powerTipminiemail'); });
		jQuery('#miniemail').attr('class', 'input-red');
	}
	if (!validateEmail(loginUser.email)) {
		powerTip.create('miniemail', poppinres.text.emailinvalid, 'powerTipminiemail', -37, 100);
		jQuery('#miniemail').on('focusin', function() { powerTip.hide('powerTipminiemail'); });
		jQuery('#miniemail').attr('class', 'input-red');
	}
	...
	if(jQuery("#miniloginform .input-red").size() > 0) {
		return false;
	}
...
function registerformSubmitUser()
{
	...
	if(jQuery('form#mini-new-customer-register > fieldset > input#minisubscribe').is(':checked')) {
		emailsubscribe = 'T';
	}
...
function validateRegisterForm()
{
	...
	var email		= jQuery('form#mini-new-customer-register > fieldset > div > input#miniemailregnew').first();
	var fname		= jQuery('form#mini-new-customer-register > fieldset > div > input#minifname').first();
	var lname		= jQuery('form#mini-new-customer-register > fieldset > div > input#minilname').first();
	var pass		= jQuery('form#mini-new-customer-register > fieldset > div > input#minipasswordnew').first();
	
	//remove unnecessary spaces before and after name values 
	jQuery('form#mini-new-customer-register > fieldset > div > input#minifname').val(jQuery('form#mini-new-customer-register > fieldset > div > input#minifname').val().trim());
	jQuery('form#mini-new-customer-register > fieldset > div > input#minilname').val(jQuery('form#mini-new-customer-register > fieldset > div > input#minilname').val().trim());
	
	jQuery("#mini-new-customer-register .input-red").removeClass("input-red");
	
	if(!regEmail.test(email.val()))
	{
		powerTip.create('miniemailregnew', poppinres.text.emailvalidation, 'powerTipEmail', -52, 97);
		jQuery('#miniemailregnew').on('focusin', function() { powerTip.hide('powerTipEmail'); }) ;
		email.attr( "class", "input-red");
	}
	if(!regText.test(fname.val()))
	{
		powerTip.create('minifname', poppinres.text.firstnamevalidation, 'powerTipFName', -52, 97);
		jQuery('#minifname').on('focusin', function() { powerTip.hide('powerTipFName'); }) ;
	...
	if(!regText.test(lname.val()))
	{
		...
		jQuery('#minilname').on('focusin', function() { powerTip.hide('powerTipLName'); }) ;
		lname.attr( "class", "input-red");
	}
	if(!regPass.test(pass.val()))
	{
		powerTip.create('minipasswordnew', poppinres.text.passwordvalidation, 'powerTipPass', -65, 97);
		jQuery('#minipasswordnew').on('focusin', function() { powerTip.hide('powerTipPass'); }) ;
		pass.attr( "class", "input-red");
	}
	
	if(jQuery("form#mini-new-customer-register input.input-red").size() > 0) {
		return false;
	} else {
			return true;
	}
