	...
	,	addCrossDomainParameters: function (url)
		{
			...
			_.each(this.trackers, function (tracker)
			{
				...
				{
					url = tracker.addCrossDomainParameters.call(self, url);
				}
			});
...
(function (application)
{
	...
	,	navigationTabs: [
		...
		, categories: [
						{
								text: _('Home Sub-menu 1').translate()
							, 	href: '/Decor_2'
							, 	data: {
										touchpoint: 'home'
									, 	hashtag: '#Decor_2'
					...
					, 	{
							text: _('Home Sub-menu 2').translate()
						, 	href: '/submenu2'
						, 	data: {
									touchpoint: 'home'
								, 	hashtag: '#submenu2'
...
,	function (DataModels, EnhancedViews, LandingPages)
{
	...
	return {
		...
		mountToApp: function (Application)
		{
			...
			Layout.showContent = _.wrap(Layout.showContent, function (fn, view, dont_scroll)
			{
				...
				DataModels.loadPage('/' + Backbone.history.fragment, function (page)
				{
					...
					fn.call(Layout, view, dont_scroll).done(function ()
					{
	...
	,	addCrossDomainParameters: function (url)
		{
			...
			if (!~url.indexOf(this.getConfig('tracking.google.domainName')))
			{
				_gaq.push(function ()
				{
		...
		,	trackEvent: function (event)
			{
				...
				win[name]('send', 'event', event.category, event.action, event.label, event.value, {
		...
		,	trackTransaction: function (order)
			{
				if (order && order.get('confirmation'))
				{
					var transaction_id = order.get('confirmation').internalid
					,	order_summary = order.get('summary')
					,	item = null;
		...
		,	addCrossDomainParameters: function (url)
			{
				// We only need to add the parameters if the url we are trying to go
				// is not a sub domain of the tracking domain
				if (!~url.indexOf(this.getConfig('tracking.googleUniversalAnalytics.domainName')))
				{
					win[name](function (tracker)
					{
						window.linker = window.linker || new window.gaplugins.Linker(tracker);

						var track_url = window.linker.decorate(url);
						// This validation is due to Tracking Blockers overriding the default anlaytics methods
						if (typeof track_url === 'string')
						{
							url = track_url;
		...
		,	mountToApp: function (application)
			{
				...
				if (tracking && tracking.propertyID)
				{
					...
					application.trackers.push(GoogleUniversalAnalytics);
				}	
			}
		};
...
define('Merchandising.Context', function ()
{
	...
	,	getFilters: function (filters, isWithin)
		{
			return this.callHandler('getFilters', filters, isWithin) || _.map(filters, function (values, key)
			{
				if (values.length)
				{
					filters[key] = values.join(',');
...
,	function (MerchandisingItemCollection, MerchandisingRule, MerchandisingContext)
{
	...
	var MerchandisingZone = function MerchandisingZone (element, options)
	{
		...
		if (this.model && !this.$element.hasClass(this.loadingClassNames))
		{
	...
	var NavigationHelper = {
		...
		mountToApp: function (application)
		{
			...
			,	getTargetTouchpoint: function ($target)
				{
					...
					,	new_url = ''
					,	clean_hashtag = hashtag && hashtag.replace('#/', '').replace('#', ''); 
					...
					{
						new_url = clean_hashtag ? ( '/' + clean_hashtag ) : $target.attr('href');
			...
			,	touchpointMousedown: function (e)
				{
					...
					if (!$target.data('fixed-href') || $target.data('keep-options'))
					{
			...
			,	getKeepOptionsUrl: function($target) 
				{
					if (_.getWindow().location.href.indexOf('?') > 0 && $target.data('keep-options'))
					{
						...
						if($target.data('keep-options') !== '*')
						{
							...
							_(_(current_options).keys()).each(function(key){ 
						...
						var anchor_options = ($target.attr('href') && $target.attr('href').indexOf('?') > 0) ? _.parseUrlOptions($target.attr('href')) : {}; 
						anchor_options = _.extend(anchor_options, current_options); //override with global
						var href_to_fix = $target.attr('href') || ''; 
			...
			,	keepOptionsMousedown: function(e) 
				{
					var $target = jQuery(e.currentTarget);
					var new_url = this.getKeepOptionsUrl($target);
					$target.attr('href', new_url); 
				}
			});
	...
	,	getRelatedItems: function ()
		{
			...
			// TODO: add support for sites type standard
			if (lines.models && SC.ENVIRONMENT.siteSettings.sitetype === 'ADVANCED')
			{
	...
	,	handleMarkError: function ($container)
		{
			...
			$container.find('[data-type="alert-placeholder"]').html(
...
define('SocialSharing', function ()
{
	...
	// TODO: Delete this since it's been deprecated, openGraphMapping replaced this
	...
	function shareInPinterestEventListener (e)
	{
		...
		var openGraphMapping = this.getApplication().getConfig('openGraphMapping')
		,	url = openGraphMapping.url(this, 'pinterest')
		,	image = openGraphMapping.image(this, 'pinterest')
		,	description = openGraphMapping.description(this, 'pinterest');
	...
	function shareInTwitterEventListener (e)
	{
		...
		var openGraphMapping = this.getApplication().getConfig('openGraphMapping')
		,	url = openGraphMapping.url(this, 'twitter')
		,	title = openGraphMapping.title(this, 'twitter')
	...
	function refreshFacebookElements ()
	{
		...
		,	openGraphMapping = this.getApplication().getConfig('openGraphMapping')
		,	pluginOptions = this.getApplication().getConfig('facebook.pluginOptions')
		,	url = openGraphMapping.url(this, 'facebook');
	...
	function shareInGooglePlusEventListener (e)
	{
		e.preventDefault();
		var openGraphMapping = this.getApplication().getConfig('openGraphMapping')
		,	url = openGraphMapping.url(this, 'google-plus');
	...
	// setOpenGraphTags:
	// Based on the openGraphMapping, sets the open graph tags, 
	function setOpenGraphTags ()
	{
		var self = this;
		_.each(this.getApplication().getConfig('openGraphMapping'), function (fn, name)
		{
			var value = fn(self);
			if (!open_graph_tags[name])
			{
				if (jQuery('meta[property="og:'+name+'"]').length)
				{
					open_graph_tags[name] = jQuery('meta[property="og:'+name+'"]');
				}
				else
				{
					open_graph_tags[name] = jQuery('<meta />', {property: 'og:'+name});
					jQuery('head').append(open_graph_tags[name]);
				}
			}
			open_graph_tags[name].attr('content', value || '');
		});
	}
	...
	function refreshAddThisElements ()
	{
		...
		if (window.addthis)
		{
			...
			var share_options = {
				url: openGraphMapping.url(this, 'add-this')
			,	title: openGraphMapping.title(this, 'add-this')
			,	description: openGraphMapping.description(this, 'add-this')
			};
	...
	return {
		mountToApp: function (Application)
		{
			...
			Layout.on('afterAppendView', function ()
			{
				...
				// Then we set the openGraph tags
				Layout.setOpenGraphTags();
			});
		}
	};
});
