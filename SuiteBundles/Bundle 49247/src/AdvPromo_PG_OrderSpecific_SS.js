...
function getPromotionDiscount(promotionsInput, promotionsOutput){
	...
	try{
		advPromoLogger('DEBUG', 'using Order Specific plugin');
		...
		var isApplyToAllOrders = isAnyOrder(promoId);
...
//			}
		...
		if(isEligible){
			...
			applyOrderDiscount(promotionsInputModel, promotionsOutput, mainModel);
		}
		else{
...
function getName(locale, NLSingleStringOutput){
	NLSingleStringOutput.setString('Order-Based Promotions');
}

function getFormName(nullInput, NLSingleStringOutput){
	NLSingleStringOutput.setString('Order Specific Promotion Form');
}
