...
var TRANS_CS_IS_Order_AllItems = {};
function init_CS_IS_Order_AllItems() {
	var translates_CS_IS_Order_AllItems = [];
	translates_CS_IS_Order_AllItems.push(new TranslateMember('label.percentoff', 'LABEL_PERCENT_OFF', 'Percent Off'));
	translates_CS_IS_Order_AllItems.push(new TranslateMember('label.amount.set', 'LABEL_SET_AMOUNT', 'Set Amount'));
	translates_CS_IS_Order_AllItems.push(new TranslateMember('label.purchase.min', 'LABEL_MIN_PURCHASE', 'Minimum Purchase'));
	translates_CS_IS_Order_AllItems.push(new TranslateMember('text.or', 'TEXT_OR', 'or'));
	translates_CS_IS_Order_AllItems.push(new TranslateMember('text.on', 'TEXT_ON', 'on'));
	translates_CS_IS_Order_AllItems.push(new TranslateMember('text.units', 'TEXT_UNITS', 'Units'));
	translates_CS_IS_Order_AllItems.push(new TranslateMember('text.order.total', 'TEXT_ORDER_TOTAL', 'Order Total'));
	translates_CS_IS_Order_AllItems.push(new TranslateMember('error.minpurch.enter', 'ERROR_MINPURCH_ENTER', 'Please enter value(s) for: Minimum Purchase'));
	translates_CS_IS_Order_AllItems.push(new TranslateMember('error.minpurch.nan', 'ERROR_MINPURCH_NAN', 'Invalid Minimum Purchase value. Values must be numbers.'));
	translates_CS_IS_Order_AllItems.push(new TranslateMember('error.minpurch.range', 'ERROR_MINPURCH_RANGE', 'Invalid Minimum Purchase value. Values must be between 0 to 100,000,000,000'));
	TRANS_CS_IS_Order_AllItems = new TranslateHelper(translates_CS_IS_Order_AllItems);
}

var TranslateInitFunctions;
var TranslateInit;
if (!TranslateInitFunctions) TranslateInitFunctions = [];
TranslateInitFunctions.push(init_CS_IS_Order_AllItems);
...
function pageInitAllItems(type) {
	...
	var currencyList = [{"value":"0", "text":TRANS_CS_IS_Order_AllItems.TEXT_UNITS}];
	var id = null;
	var name = null;
    ...
    if(formName != 'Buy X Get Y Promotion Form'){
    	if(multicurrencyON){
    		...
    		currencyList = [{"value":"0", "text":TRANS_CS_IS_Order_AllItems.TEXT_UNITS}];
...
//    		currencyList.push({"value":1, "text":recCurrency.getFieldValue('symbol')});
	...
	var theLabel = new LabelWithHelp(TRANS_CS_IS_Order_AllItems.LABEL_MIN_PURCHASE, 'custpage_advpromo_minpurchase');
...
function addItem() {
		...
		for (var i = 0; i < currencies.length; i++) {
			if (!(isNumeric.test(currencies[i].value))) {
				...
				if ((currencies[i].value == '') || (currencies[i].value == null)) {
					message =  TRANS_CS_IS_Order_AllItems.ERROR_MINPURCH_ENTER;
				} else {
					message =  TRANS_CS_IS_Order_AllItems.ERROR_MINPURCH_NAN;
				}
				window.alert(message);
				return;
			} else {
				if (!((currencies[i].value > 0) && (currencies[i].value < 100000000000) )) {
					window.alert(TRANS_CS_IS_Order_AllItems.ERROR_MINPURCH_RANGE);
					return;
				} 
			}
		}
	...
	orders.label = TRANS_CS_IS_Order_AllItems.TEXT_ORDER_TOTAL; //'All Items';
