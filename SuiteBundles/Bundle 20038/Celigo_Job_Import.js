function beforeLoad(type, form) {
	...
	if (nlapiGetFieldValue('custrecord_celigo_ji_status') != '2' && nlapiGetFieldValue('custrecord_celigo_ji_status') != '1')
		return;
	
	if (nlapiGetFieldValue('custrecord_celigo_ji_log') == null || nlapiGetFieldValue('custrecord_celigo_ji_log') == '')
		return;
	
	var errors = getAllErrors(nlapiGetFieldValue('custrecord_celigo_ji_log'));
	if (errors == null || errors.length < 1)
		return;
	
	var error = form.addTab('custpage_celigo_ji_error', 'Errors');
	var errorList = form.addSubList('custpage_celigo_ji_errors', 'staticlist', 'Errors', 'custpage_celigo_ji_error');
	errorList.addField('custpage_celigo_ji_errors_id', 'text', 'Error Id');
	errorList.addField('custpage_celigo_ji_errors_date', 'text', 'Date Created');
	errorList.addField('custpage_celigo_ji_errors_msg', 'textarea', 'Error Message');
	errorList.addField('custpage_celigo_ji_errors_link', 'text', 'Link');
	errorList.addField('custpage_celigo_ji_errors_res', 'text', '');
	
	for (var i=0; i<errors.length; i++) {
		var url = nlapiResolveURL('RECORD', 'customrecord_celigo_logger', errors[i].getId(), false);
		errorList.setLineItemValue('custpage_celigo_ji_errors_id', i+1, "<a href='" + url + "'>" + errors[i].getId() + "</a>");
		errorList.setLineItemValue('custpage_celigo_ji_errors_date', i+1, errors[i].getValue('created'));
		errorList.setLineItemValue('custpage_celigo_ji_errors_msg', i+1, errors[i].getValue('custrecord_celigo_logger_message'));
		...
		if (errors[i].getValue('custrecord_celigo_logger_rec_type') != null && errors[i].getValue('custrecord_celigo_logger_rec_type') != '' &&
			...
			errorList.setLineItemValue('custpage_celigo_ji_errors_link', i+1, "<a href='" + url + "'>" + errors[i].getValue('custrecord_celigo_logger_rec_type') + "</a>");
		}
		...
		errorList.setLineItemValue('custpage_celigo_ji_errors_res', i+1, "<a href='" + url + "'>Mark Resolved</a>");
	}
